...
inline void legend(const std::map<std::string, std::string> &keywords) {
  ...
  for (std::map<std::string, std::string>::const_iterator it = keywords.begin();
       it != keywords.end(); ++it) {
    // 处理整数型参数
    if (it->first == "ncol" || it->first == "columnspacing" ||
        it->first == "handlelength" || it->first == "handletextpad" ||
        it->first == "borderpad") {
      PyDict_SetItemString(kwargs, it->first.c_str(),
                           PyLong_FromLong(std::stol(it->second)));
    }
    // 处理浮点型参数
    else if (it->first == "fontsize" || it->first == "framealpha" ||
             it->first == "shadow") {
      PyDict_SetItemString(kwargs, it->first.c_str(),
                           PyFloat_FromDouble(std::stod(it->second)));
    }

...
namespace libgp
{
  ...
  class CovarianceFunction
  {
    public:
      ...
      /** Covariance gradient of two input vectors with respect to the first inputs.
       *  @param x1 first input vector
       *  @param x2 second input vector
       *  @param grad_input covariance gradient with respect to the inputs */
      virtual void grad_wrt_x1(const Eigen::VectorXd &x1, const Eigen::VectorXd &x2, Eigen::VectorXd &grad_input);

      /** Covariance gradient of two input vectors with respect to the inputs.
       *  @param x1 first input vector
       *  @param x2 second input vector
       *  @param grad_input covariance gradient with respect to the inputs */
      virtual void hessian_wrt_x1_x2(const Eigen::VectorXd &x1, const Eigen::VectorXd &x2, Eigen::MatrixXd &hessian_input);
      ...
      /* Return lower bounds in log space for the hyperparameters */
      virtual Eigen::VectorXd get_loghyper_lb();

       /* Return upper bounds in log space for the hyperparameters */
      virtual Eigen::VectorXd get_loghyper_ub();

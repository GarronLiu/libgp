...
void CG::maximize(GaussianProcess *gp, size_t n, bool verbose) {
  ...
  */
  ...
  /* The code falls naturally into 3 parts, after the initial line search is
     ...
     until we have extrapolated far enough (Wolfe-Powell conditions). 2) if
     necessary, we enter the second loop which takes p2, p3 and p4 chooses the
     subinterval containing a (local) minimum, and interpolates it, unil an
     acceptable point is found (Wolfe-Powell conditions). Note, that points are
     always maintained in order p0 <= p1 <= p2 < p3 < p4. 3) compute a new
     search direction using conjugate gradients (Polack-Ribiere flavour), or
     revert to steepest if there was a problem in the previous line-search.
     Return the best value so far, if two consecutive line-searches fail, or
     whenever we run out of function evaluations or line-searches. During
     extrapolation, the "f" function may fail either with an error or returning
     Nan or Inf, and maxmize should handle this gracefully.
  */
  ...
  Eigen::VectorXd X = gp->get_hyperparameters();        // hyper parameters

  if (df0.norm() < tol_) {
    if (verbose)
      cout << "CG converged at start with gradient norm " << df0.norm() << endl;
    lml_ = -f0;
    duration_ = 0;
    return;
  }
  ...
  lml_history_.clear();
  lml_history_.reserve(n);
  time_cost_history_.clear();
  time_cost_history_.reserve(n);

  for (unsigned int i = 0; i < n; ++i) {
    lml_history_.push_back(-f0);
    // 新增：在每次迭代开始时检查梯度范数
    if (df0.norm() < tol_) {
      if (verbose)
        cout << "CG converged at iteration " << i << " with gradient norm "
             << df0.norm() << endl;
      break;
    }
    ...
    while (1) // keep extrapolating until necessary
    {
      ...
      while (!success && M > 0) {
        M--;
        i++;
        gp->update_hyperparameters(X + s * x3);
      ...
      double x1 = x2;
      double f1 = f2;
      double d1 = d2; // move point 2 to point 1
      x2 = x3;
      f2 = f3;
      d2 = d3; // move point 3 to point 2
      double A =
          6 * (f1 - f2) + 3 * (d2 + d1) * (x2 - x1); // make cubic extrapolation
      double B = 3 * (f2 - f1) - (2 * d1 + d2) * (x2 - x1);
      x3 = x1 -
           d1 * (x2 - x1) * (x2 - x1) / (B + sqrt(B * B - A * d1 * (x2 - x1)));
      if (isnan(x3) || x3 < 0 ||
          x3 > x2 * EXT) // num prob | wrong sign | beyond extrapolation limit
        x3 = EXT * x2;
    ...
    if ((abs(d3) < -SIG * d0) && (f3 < f0 + x3 * RHO * d0)) {
      X = X + s * x3;
      f0 = f3;
      s = (df3.dot(df3) - df0.dot(df3)) / (df0.dot(df0)) * s -
          df3;   // Polack-Ribiere CG direction
      ...
      x3 = x3 *
           std::min(
               RATIO,
               d3 / (d0 - std::numeric_limits<double>::min())); // slope ratio
                                                                // but max RATIO
      ls_failed = false; // this line search did not fail
    } else {             // restore best point so far
      X = X0;
      f0 = F0;
      df0 = dF0;
    ...
    duration_ = std::chrono::duration<double, std::milli>(iter_end - iter_start).count();
    time_cost_history_.push_back(duration_);
  }
  auto end = std::chrono::high_resolution_clock::now();
  duration_ = std::chrono::duration<double, std::milli>(end - start).count();
  lml_ = -gp->log_likelihood();

...
def main():
    ...
    sgp_rmse_list = {s: [run['rmse'][s] for run in sgp_runs] for s in state_names}
    sgp_mae_list = {s: [run['mae'][s] for run in sgp_runs] for s in state_names}
    
    # 计算均值和最大最小值
    sgp_rmse_mean = {s: np.mean(sgp_rmse_list[s]) for s in state_names}
    sgp_rmse_std = {s: np.std(sgp_rmse_list[s]) for s in state_names}
    sgp_rmse_max = {s: np.max(sgp_rmse_list[s]) for s in state_names}
    sgp_rmse_min = {s: np.min(sgp_rmse_list[s]) for s in state_names}
    sgp_mae_mean = {s: np.mean(sgp_mae_list[s]) for s in state_names}
    sgp_mae_std = {s: np.std(sgp_mae_list[s]) for s in state_names}
    sgp_mae_max = {s: np.max(sgp_mae_list[s]) for s in state_names}
    sgp_mae_min = {s: np.min(sgp_mae_list[s]) for s in state_names}
    ...
    sgp_rmse_str = " ".join([f"{sgp_rmse_mean[s]:6.4f}" for s in state_names])
    print(f"{'SGP-DE(Mean)':<15} {sgp_rmse_str}")
    sgp_rmse_str = None
    sgp_rmse_str = " ".join([f"{sgp_rmse_max[s]:6.4f}" for s in state_names])
    print(f"{'SGP-DE(Max)':<15} {sgp_rmse_str}")
    sgp_rmse_str = None
    sgp_rmse_str = " ".join([f"{sgp_rmse_min[s]:6.4f}" for s in state_names])
    print(f"{'SGP-DE(Min)':<15} {sgp_rmse_str}")
    ...
    sgp_mae_str = " ".join([f"{sgp_mae_mean[s]:6.4f}±{sgp_mae_std[s]:.4f}" for s in state_names])
    print(f"{'SGP-DE(Mean±Std)':<15} {sgp_mae_str}")

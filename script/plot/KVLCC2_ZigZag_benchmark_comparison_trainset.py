...
prediction_files = [
    "KVLCC2_ZigZag_KPM_TrainingSet_prediction.csv",
    "KVLCC2_ZigZag_nuSVR_TrainingSet_prediction.csv",
    "KVLCC2_ZigZag_DE_RandomID=2_TrainingSet_prediction.csv",
    "KVLCC2_ZigZag_CG_RandomID=6_TrainingSet_prediction.csv"
]
states = [3, 4, 5]
...
# 配置图保存参数
save_fig = True
fig_dir = "result/convergence_analysis_figures/ZigZag_benchmark"
save_format = "svg"
dpi = 300
...
def plot_accuracy_comparison(title):
    ...
    # 读取真实数据集
    true_train_filepath = os.path.join(base_dir, "TrainingSet_Smoothed.csv")
    ...
    for state_idx, label in enumerate(label_list):
        ...
        for benchmark in benchmarks:
            if predictions[benchmark] is not None:
                ...
                ax.plot(time_data,pred_data, color=colors[benchmark], label=f'{benchmark} (RMSE: {rmse:.4f})', linestyle=lines[benchmarks.index(benchmark)])
        if state_idx // 3 == 1:
    ...
    plt.show()
    if save_fig:
        save_path = os.path.join(fig_dir, f"accuracy_comparison_timeseries_trainset.{save_format}")
    ...
    plt.show()
    if save_fig:
        save_path = os.path.join(fig_dir, f"accuracy_comparison_trajectory_trainset.{save_format}")
        fig.savefig(save_path, format=save_format, dpi=dpi, bbox_inches='tight')
        print(f"Figure saved to {save_path}")
